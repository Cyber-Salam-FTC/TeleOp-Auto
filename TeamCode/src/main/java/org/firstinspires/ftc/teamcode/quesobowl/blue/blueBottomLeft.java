package org.firstinspires.ftc.teamcode.quesobowl.blue;
...
@Autonomous
public class blueBottomLeft extends LinearOpMode {
    ...
    boolean initMidpointComplete = false;

    Pose startPose = new Pose(15, 8, Math.toRadians(180));
    Pose scorePose = new Pose(97, 68, Math.toRadians(270));
    Pose intake1HeadingPose = new Pose(60, 33, Math.toRadians(180));
    Pose intake1DeepPose = new Pose(20, 33, Math.toRadians(180));
    Pose intake2HeadingPose = new Pose(60, 57, Math.toRadians(180));
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Midpoint?: ", initMidpointComplete);
            telemetry.update();
        }
    }
    ...
    public void runStateMachine() {
        switch (state) {
            case GO_TO_SHOOT:
                if (!pathStarted) {
                    if (!initMidpointComplete) {
                        follower.followPath(follower.pathBuilder()
                                .addPath(new BezierCurve(follower.getPose(), midpoint, scorePose))
                                .setLinearHeadingInterpolation(follower.getHeading(), scorePose.getHeading())
                                .build());
                        initMidpointComplete = true;
                    } else {
                        follower.followPath(follower.pathBuilder()
